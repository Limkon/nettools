...
// [New] 显示归属地复选框
#define ID_CHK_SHOW_LOCATION 117
...
void start_task(TaskType type) {
    ...
    // [New] 获取是否显示归属地
    p->showLocation = (IsDlgButtonChecked(hMainWnd, ID_CHK_SHOW_LOCATION) == BST_CHECKED);
    ...
    if (type == TASK_PING) {
        ...
        for(int i=0; i<5; i++) {
            LVCOLUMNW lvc = {0}; lvc.mask = LVCF_TEXT | LVCF_WIDTH; lvc.pszText = cols[i]; lvc.cx = (i==0?180:90);
        ...
        LVCOLUMNW lvc = {0}; lvc.mask = LVCF_TEXT|LVCF_WIDTH; lvc.pszText = L"提取到的IP地址"; lvc.cx = 200;
        ListView_InsertColumn(hList, 0, &lvc);
        if (p->showLocation) {
            lvc.pszText = L"归属地"; lvc.cx = 200;
    ...
    case WM_CREATE:
        {
            ...
            // [New] 显示归属地 复选框
            CreateWindowW(L"BUTTON", L"显示IP归属地 (需纯真IP库)", WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX, 680, grp1Y+85, 180, 20, hWnd, (HMENU)ID_CHK_SHOW_LOCATION, hInst, NULL);
